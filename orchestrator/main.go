...
func restartProcesses() {
	ffmpegCmd := exec.Command("tmux", "send-keys", "-t", "ffmpeg",
		`ffmpeg -f rawvideo -pix_fmt bgr24 -s 1280x720 -r 30 -i /tmp/video_pipe2 -c:v vp8 -b:v 3M -g 15 -an -filter:v "drawtext=text='%{localtime}':x=10:y=10:fontcolor=white:fontsize=14:box=1:boxcolor=black@0.5" -preset ultrafast -f rtp rtp://127.0.0.1:5005`,
		"C-m")
	ffmpegCmd.Start()
